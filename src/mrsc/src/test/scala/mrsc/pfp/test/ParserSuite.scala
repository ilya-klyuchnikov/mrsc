...
class ParserSuite extends org.scalatest.FunSuite {
  ...
  test("parsing case expression") {
    ...
    testTermParsing("""*case Nil() of { Nil() -> Nil() ; Cons(a, b) -> Cons(a, b) }""",
      Case(Ctr("Nil", List()), List((Ptr("Nil", List()), Ctr("Nil", List())), (Ptr("Cons", List("a", "b")), Ctr("Cons", List(BVar(1), BVar(0))))), 1))
    testTermParsing("""*case Nil() of { Nil() -> *Nil() ; Cons(a, b) -> *Cons(a, b) }""",
      Case(Ctr("Nil", List()), List((Ptr("Nil", List()), Ctr("Nil", List(), 1)), (Ptr("Cons", List("a", "b")), Ctr("Cons", List(BVar(1), BVar(0)), 1))), 1))
    testTermParsing("""*case *Nil() of { Nil() -> *Nil() ; Cons(a, b) -> *Cons(a, b) }""",
      Case(Ctr("Nil", List(), 1), List((Ptr("Nil", List()), Ctr("Nil", List(), 1)), (Ptr("Cons", List("a", "b")), Ctr("Cons", List(BVar(1), BVar(0)), 1))), 1))
  }
  ...
  test("parsing case expression inside lambda") {
    ...
    testTermParsing("""\x -> case *x of {Cons(y, ys) -> x}""", Abs(Case(BVar(0, 1), List((Ptr("Cons", List("y", "ys")), BVar(2))))))
    testTermParsing("""*\x -> case *x of {Cons(y, ys) -> x}""", Abs(Case(BVar(0, 1), List((Ptr("Cons", List("y", "ys")), BVar(2)))), 1))
  }
  ...
  test("parsing lets") {
    ...
    testTermParsing("""let x = Nil() in *x""",
      Let(Ctr("Nil", List()), BVar(0, 1)))
    testTermParsing("""let x = **Nil() in *x""",
      Let(Ctr("Nil", List(), 2), BVar(0, 1)))
  ...
  test("parsing bindings with ticks") {
    val in =
      """app = \x -> \y ->
        ...
        |    };""".stripMargin
    val expected = Map("app" ->
      Abs(Abs(Case(BVar(1, 1), List(
        (Ptr("Nil", List()), BVar(0, 1)),
        ((Ptr("Cons", List("x", "xs")), Ctr("Cons", List(BVar(1), App(App(GVar("app"), BVar(0)), BVar(2), 2) ))))), 2))))
    testBindingsParsing(in, expected)
  }
