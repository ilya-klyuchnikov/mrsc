...
case class DSC(val gc: GContext) extends PFPRules
  with PFPSyntax
  with PFPSemantics
  with Driving
  with Folding
...
class Deforestation2Suite extends FunSuite {
  ...
  def testExample(bindings: GContext, goal: Term, expectedResult: Term): Unit = {
    ...
    val tGraph = Transformations.transpose(graphs.head)
    info(tGraph.toString())
    val result = Residuator2(tGraph).result
    info(result.toString())
