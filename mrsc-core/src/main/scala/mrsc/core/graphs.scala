...
/* # SC Graph Abstraction
 ...
 In MRSC SC Graph is a special kind of a graph that can be seen as a tree (skeleton),
 some leaves of which can have loopbacks.
 Note that loopbacks can start only from leaves.

 In a sense, SC Graph is similar to accessible
 ...
 SC graphs in MRSC have two representations:

 * `SGraph` (stack graph) - convenient for bottom-up traversals and
    for using in multi-result supercompilation
    (a graph consists of complete and incomplete parts,
    and adding new elements to incomplete parts is cheap).
    This data structure is very similar to "spaghetti stack" - http://en.wikipedia.org/wiki/Spaghetti_stack
 * `TGraph` (tree graph) - convenient for top-down traversals (a node knows about its outs).

 The main idea here is to use functional (immutable) data structures in order to support
 multi-result values composed of shared data.
 */
...
/** `Graph[C, D, E]` is a core data structure in MRSC.
  * It may represent (1) a "work in progress" and (2) a complete graph.
  * We know already processed part of an SC graph
  * (`completeLeaves`, `completeNodes`) and the frontier
  * of incomplete part (`incompleteLeaves`).
  *
  * `C` (configuration) is a type of node labels;
  * `D` (driving) is a type of edge labels (driving info);
  */
...
/*! `TNode[C, D, E]` is a minimal and straightforward implementation of
 * a top-down node.
 */
...
/*! `TGraph[C, D, E]`.
 * `TGraph` is a representation of a graph of configurations
 * that is convenient for top-down traversals (a node knows about its `outs`).
 *
 ...
 *`C` (configuration) is a type of node labels;
 *`D` (driving) is a type of edge labels (driving info);
 *`E` (extra information) is a type of extra labels (extra info).
...
/*! Auxiliary data structure for transposing a graph into a transposed graph.
 */
...
object Transformations {
  /*! Transposition is done in the following way. Nodes are grouped according to the
   levels (the root is 0-level). Then graphs are produced in a bottom-up fashion.
   */
